...
import type { StatsData } from "../_components/dashboard-client";
...
export async function fetchStats(range: string): Promise<StatsData> {
  ...
  for (const guide of guides) {
    ...
    if (guide.inroScenarioIds?.length) {
      pageScenarioIds[key] = guide.inroScenarioIds;
    }
    if (guide.date) pageDates[key] = guide.date;
  }
  ...
  const totals = {
    ...
    rate: totalStats.dms > 0 ? ((totalStats.clicks / totalStats.dms) * 100).toFixed(1) : "0",
  ...
  return { range, stats, daily, totals };
}
